...
class PM25_Dataset(Dataset):
    """
    A Dataset class for loading and processing PM2.5 air quality data for use with PyTorch models.
    
    Parameters:
    - eval_length (int): The length of the time series evaluation window (total number of timestamps (L)).
    - target_dim (int): The total number of the different features in the multivariate time series (K).
    - mode (str): Specifies the mode of the dataset. Can be 'train', 'valid', or 'test'.
    - validindex (int): Index used to select the validation month in 'train' and 'valid' modes.
    """
    ...
    def __getitem__(self, org_index):
        ...
        Parameters:
        - org_index (int): The original index of the sample to retrieve.
        
        Returns:
        - dict: A dictionary containing the data sample including observed data, masks, and timepoints, and a mask to avoid double evaluation of overlapping values.
        """
...
def get_dataloader_pm25(batch_size, device, validindex=0):
    """
    Prepares DataLoader objects for the PM2.5 dataset for training, validation, and testing. Also, returns 
    normalization parameters (scaler and mean_scaler) to normalize the data.
    
    Parameters:
    - batch_size (int): Batch size for the DataLoader.
    - device (torch.device): The device on which the tensors will be loaded.
    - validindex (int): Index used to select the validation month in 'train' and 'valid' modes.
    
    Returns:
    - Tuple: Contains DataLoader objects for training, validation, and testing, along with normalization parameters (scaler, mean_scaler).
    """
